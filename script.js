...
function renderSidebarItem(article) {
    ...
    return `
        <article class="sidebar-item" onclick="window.location.href='/articles/${article.slug}'" style="cursor:pointer;">
...
function renderGridCard(article) {
    const img = optimizeImage(article.cover_image || FALLBACK_IMAGE, 500);
    return `
        <article class="small-card card" onclick="window.location.href='/articles/${article.slug}'" style="cursor:pointer;">
            <img src="${img}" alt="${article.title}" loading="lazy">
...
function renderExclusiveCard(article) {
    ...
    return `
        <article class="exclusive-card card" onclick="window.location.href='/articles/${article.slug}'" style="cursor:pointer;">
            <img src="${img}" alt="${article.title}" loading="lazy">
            <div class="card-content">
...
async function initHomePage() {
    ...
    if (heroContainer && articles.length > 0) {
        ...
        heroContainer.innerHTML = `
            <article class="main-card card" onclick="window.location.href='/articles/${heroArt.slug}'" style="cursor:pointer;">
...
async function initHomeCategoryReviews() {
    ...
    if (movieSlider) {
        if (mReviews.length > 0) {
            ...
            const renderSlide = (idx) => {
                ...
                return `
                    <div class="movie-slider card" onclick="window.location.href='/articles/${rev.slug}'" style="cursor:pointer;">
    ...
    if (tvList) {
        tvList.innerHTML = tvReviews.length > 0 ? tvReviews.map(rev => {
            const img = optimizeImage(rev.cover_image || FALLBACK_IMAGE, 500);
            return `
                <article class="review-item" onclick="window.location.href='/articles/${rev.slug}'" style="cursor:pointer;">
                    <img src="${img}" alt="${rev.title}" loading="lazy">
...
async function initTopStoriesPage() {
    ...
    let sideStoriesHtml = sideStories.map(a => `
        <div class="side-article" onclick="window.location.href='/articles/${a.slug}'">
    ...
    heroContainer.innerHTML = `
        <div class="hero-article" onclick="window.location.href='/articles/${hero.slug}'">
    ...
    if (gridStories.length > 0) {
        gridContainer.innerHTML = gridStories.map(a => `
            <div class="article-card" onclick="window.location.href='/articles/${a.slug}'">
                <img src="${optimizeImage(a.cover_image || FALLBACK_IMAGE, 500)}" alt="${a.title}" loading="lazy">
...
async function initArticlePage() {
    ...
    const articleUrl = `https://netcinemanews.live//articles/${article.slug}`;
    ...
    if (movieNewsContainer) {
        ...
        if (latestMovieNews.length > 0) {
            ...
            movieNewsContainer.innerHTML = `
                <div class="breaking-card" onclick="window.location.href='/articles/${news.slug}'" style="cursor:pointer;">
    ...
    if (latestPostsContainer) {
        ...
        latestPostsContainer.innerHTML = latestPosts.map(post => {
            ...
            return `
                <div class="popular-item" onclick="window.location.href='/articles/${post.slug}'" style="cursor:pointer;">
    ...
    if (relatedContainer) {
        ...
        if (related.length > 0) {
            relatedContainer.innerHTML = related.map(r => `
                <div class="related-card" onclick="window.location.href='/articles/${r.slug}'" style="cursor:pointer;">
    ...
    if (prevPostElem && currentIndex < catArticles.length - 1 && currentIndex !== -1) {
        ...
        prevPostElem.href = `/articles/${prevPost.slug}`;
    ...
    if (nextPostElem && currentIndex > 0) {
        ...
        nextPostElem.href = `/articles/${nextPost.slug}`;
